...
let questions=reactive([]) // 问题资料
let notifys=reactive([]) // 通知资料
let question_box=ref(true)
let notify_box=ref(false)
let optionText=ref('') // 临时选项文字站存区
...
<template>
<div @click="closeAll()" class="container preview page">
    <!-- header -->
    <header>
                ...
                <div class="page-content">
                    ...
                    <div class="items-wrap item-reply">
                        ...
                        <div class="item-content js-side-content">
                            <div v-for="(question,index) in questions" v-bind:key="question.qd_order" class="item js-item" :id="'Q'+index" :data-num=" index">
                                <div class="num">{{ index+1 }}</div>
                                <div class="preview-wrap">
                                    ...
                                    <div class="preview-content">
                                        ...
                                        <div v-if="question.qd_type == 1" class="response choose-response">
                                            <div class="edit-choose">
                                                <el-radio-group class="ml-4">
                                                    <el-radio v-for="(con,num) in question.qd_content" :label="con.text">
                                                        <span v-if="con.type == 'option'">{{ con.text }}</span>
                                                        <input v-else type="text" placeholder="其他">
                                                    </el-radio >
                                                </el-radio-group>
                                                <div>
                                                    <!-- 其他选项会由js插在这边，其他选项un active时会消失，有bug待解决 -->
                                                    <!-- 新增选项 -->
                                                    <div class="choose-item choose-add clearfix">
                                                        <div class="border-wrap">
                                                            <i class="choose-icon"></i>
                                                            <input v-on:keyup.enter="addOption(question.qd_content,index)" type="text" class="js-choose-option" placeholder="新增选项">
                                                            <div class="js-add-else">或 <a @click="addOtherOption(question.qd_content,index)" href="javascript:void(0)">新增其他</a></div>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        ...
                                        <div v-else-if="question.qd_type == 2" class="response short-response">
                                            <textarea name="" id="" class="js-textarea" placeholder="您的回答"></textarea>
                                        </div>
                                    </div>
                                </div>
                            </div>
