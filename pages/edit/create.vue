...
let edit_mod=ref('create') // 監聽目前編輯模式為新增或是修改
let od_id=ref(null) // 點擊預覽以後會送出新增問卷並生成od_id以便繼續編輯用
...
async function create(target){
    ...
    owner.od_status = 1;
    owner.create_user = user.ud_account;
    ...
    const {data,pending,error,refresh} = await useFetch(config.value+'owner-create',{
            method:"POST",
            body:dd
        });
    if(data.value.success){
        ...
        localData.upload = '新增成功！';
        sessionStorage.setItem("localData",JSON.stringify(localData));
        od_id.value = data.value.od_id;
        if(target == 'index'){
            return navigateTo('/')
        }else if(target == 'preview'){
            // 假設 previewUrl 包含了預覽頁面的 URL
            const previewUrl = '/preview/' + data.value.od_id;
            // 打開新窗口
            window.open(previewUrl, '_blank');
            return navigateTo('/edit/'+data.value.od_id)
        }
    }else{
        notification(data.value.text,'error');
        return
    }
    if(error.value){
        notification('create出錯','error');
        return
    }
}
//////////////////////// 推播框設定 ////////////////////////
        ...
        <section class="header-content">
            ...
            <div class="header-top clearfix">
                ...
                <div class="header-menu">
                    <a @click="create('index')" href="javascript:void(0)" class="btn btn-primary">
                        <FontAwesome iconName="floppy-disk"></FontAwesome>
                        <span>储存问卷</span>
                    </a>
                    <a @click="create('preview')" href="javascript:void(0)" class="btn btn-primary">
                        <FontAwesome iconName="eye"></FontAwesome>
                        <span>预览问卷</span>
                    </a>
                    <!-- <a href="javascript:void(0)" class="btn btn-delete js-btn">
